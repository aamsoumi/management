...
export default function ScatterPlotComp(Props)
{
   ...
   legend= new Set(data.map(d=>d.Kmeans_Cluster).sort((d1,d2)=>d1-d2))
    ...
    +d[yCol] >= Y2)
    {
    selected_items.push(d.articleID)
    return true
    }
      else{
         return false
    ...
    console.log(Props.selectedPapers)
    Props.setSelectedPapers([...selected_items])
    }
